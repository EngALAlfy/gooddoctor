...
class DateAppointments extends Component
{
    ...
    {
        return view('livewire.date-appointments', ['appointments' => Appointment::whereDate('date' , Carbon::parse($this->date))->where('id', 'LIKE', '%' . $this->search . '%')->orderBy('date', 'desc')->orderBy($this->orderBy, $this->order)->paginate($this->perPage)]);
    }
